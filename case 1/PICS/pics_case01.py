...
TOPK_T = 200
TOPK_MAX = 400
ADD_BAND_PER_SCAN = 200
ADD_TOPMAX_PER_SCAN = 400
...
def x0_of_y(y: torch.Tensor) -> torch.Tensor:
    return 0.5 + 0.03 * torch.sin(2.0 * PI * y) + 0.01 * torch.sin(6.0 * PI * y)
...
def s_layer(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    x0 = x0_of_y(y)
    return torch.tanh((x - x0) / DELTA)
...
def psi_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    B = 0.20
    s = s_layer(x, y)
    return A * torch.sin(PI * x) * torch.sin(PI * y) + B * s * torch.sin(PI * y)
...
def p_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    return (
        1.0
        + 0.30 * s
        + 0.05 * torch.cos(2.0 * PI * y)
        + 0.01 * torch.sin(2.0 * PI * x)
        + 0.008 * torch.sin(PI * x) * torch.sin(3.0 * PI * y)
    )
...
def phi_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    s = s_layer(x, y)
    return (
        0.50 * torch.sin(2.0 * PI * x) * torch.sin(2.0 * PI * y)
        + 0.20 * s * torch.cos(PI * y)
...
def T_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    return (
        300.0
        + 20.0 * s
        + 5.0 * torch.sin(2.0 * PI * x) * torch.cos(2.0 * PI * y)
        + 1.5 * torch.cos(PI * x) * torch.sin(3.0 * PI * y)
    )
...
def sample_band(n: int, rng: np.random.Generator) -> np.ndarray:
    n = int(n)
    y = rng.random((n,))
    x0 = 0.5 + 0.03 * np.sin(2.0 * PI * y) + 0.01 * np.sin(6.0 * PI * y)
    x = x0 + BAND_W * (2.0 * rng.random((n,)) - 1.0)
    x = np.clip(x, 0.0, 1.0)
    sel = np.stack([x, y], axis=1)  # (n,2)
    assert sel.ndim == 2 and sel.shape == (n, 2)
    return sel


def build_initial_interior_pool(nf_init: int, rng: np.random.Generator) -> np.ndarray:
    n_band = int(round(BAND_RATIO * nf_init))
    n_uni = int(nf_init - n_band)
    xy_band = sample_band(n_band, rng)
    xy_uni = sample_uniform(n_uni, rng)
...
def predict_fields_and_derivs(model: TwoExpertGated, x_in: torch.Tensor, y_in: torch.Tensor, need_param_grads: bool):
    x = x_in
    y = y_in
    create_graph = need_param_grads
    ...
    psi_xxx = grad(psi_xx, x, create_graph=create_graph)
    psi_xxy = grad(psi_xx, y, create_graph=create_graph)
    psi_xyy = grad(psi_xy, y, create_graph=create_graph)
    psi_yyy = grad(psi_yy, y, create_graph=create_graph)
    ...
    phi_x = grad(phi, x, create_graph=True)
    phi_y = grad(phi, y, create_graph=True)
    phi_xx = grad(phi_x, x, create_graph=create_graph)
    phi_yy = grad(phi_y, y, create_graph=create_graph)

    T_x = grad(T, x, create_graph=True)
    T_y = grad(T, y, create_graph=True)
    T_xx = grad(T_x, x, create_graph=create_graph)
    T_yy = grad(T_y, y, create_graph=create_graph)
...
def residuals_normalized(pred: dict, rho_e: torch.Tensor, f_u: torch.Tensor, f_v: torch.Tensor, f_T: torch.Tensor, scales: dict):
    u = pred["u"]
    v = pred["v"]

    r_u = u * pred["u_x"] + v * pred["u_y"] + pred["p_x"] - NU * (pred["u_xx"] + pred["u_yy"]) + ALPHA_E * rho_e * pred["phi_x"] - f_u
    r_v = u * pred["v_x"] + v * pred["v_y"] + pred["p_y"] - NU * (pred["v_xx"] + pred["v_yy"]) + ALPHA_E * rho_e * pred["phi_y"] - f_v
    r_phi = -EPS * (pred["phi_xx"] + pred["phi_yy"]) - rho_e
...
def main():
    ...
    interior_pool = build_initial_interior_pool(NF_INIT, rng)
    ...
    def certificate_scan_and_adapt(step: int):
        ...
        add_band = sample_band(ADD_BAND_PER_SCAN, rng)
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        with open(os.path.join(logs_dir, "loss_history.txt"), "a", encoding="utf-8") as f:
        ...
        print(f"{bar} step={step}/{MAX_STEPS} lr={lr_now:.2e} "
    ...
    metrics_path = os.path.join(logs_dir, "metrics_case01.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("PICS Case01 v3.1 metrics\n")
    ...
    runtime_path = os.path.join(logs_dir, "runtime_case01.txt")
    ...
    hist = np.loadtxt(os.path.join(logs_dir, "loss_history.txt"), skiprows=1)
    steps = hist[:, 0]
    Ltot = hist[:, 2]
    Lphys = hist[:, 3]
    Lbc = hist[:, 4]
    ...
    plot_curve(os.path.join(figs_dir, "loss_linear.png"), steps, [Ltot, Lphys, Lbc, Lcert, Lint],
               ["L_total", "L_phys", "L_bc", "L_cert", "L_int"], False, "step", "loss", "Loss history (linear)")
    plot_curve(os.path.join(figs_dir, "loss_log.png"), steps, [Ltot, Lphys, Lbc, Lcert, Lint],
               ["L_total", "L_phys", "L_bc", "L_cert", "L_int"], True, "step", "loss", "Loss history (log)")
    plot_curve(os.path.join(figs_dir, "cert_linear.png"), steps, [certs], ["cert_max"], False, "step", "cert", "Certificate history (linear)")
    plot_curve(os.path.join(figs_dir, "cert_log.png"), steps, [certs], ["cert_max"], True, "step", "cert", "Certificate history (log)")

    readme_path = os.path.join(logs_dir, "README_case01.txt")
    with open(readme_path, "w", encoding="utf-8") as f:
        f.write("PICS Case01 v3.1 README\n\n")
        f.write("Key requirements satisfied:\n")
        f.write("- No truth embedding in forward; no truth calls inside forward.\n")
        f.write("- Interior truth supervision points: 0.\n")
        f.write("- Fixed training: 200000 steps, Adam only, no early stop.\n\n")
        f.write("AxisError fix:\n")
        f.write("- band sampler uses direct construction and np.stack => always returns (n,2).\n\n")
        f.write("PICS v3.1 components:\n")
        f.write("- Two-expert gated MoE with input (x,y,xi).\n")
        f.write("- Streamfunction psi yields u,v; incompressibility holds identically.\n")
        f.write("- Constant affine scaling for p, phi, T.\n")
        f.write("- Residual normalization via RHS RMS scales; certificate uses normalized residual maxima.\n")
        f.write("- L_cert log-mean-exp proxy, L_int interface consistency, hard-buffer replay, adaptive adding.\n\n")
        f.write("Hyperparameters live directly below imports in this script.\n")
