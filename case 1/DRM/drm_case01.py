...
def residual_squares(model, xy, rho_e, f_u, f_v, f_T):
    xy_req = xy.clone().detach().requires_grad_(True)
    ...
    return {
        "J_c": W_C * DOMAIN_AREA * torch.mean(r_c * r_c),
        "J_u": W_U * DOMAIN_AREA * torch.mean(r_u * r_u),
        "J_v": W_V * DOMAIN_AREA * torch.mean(r_v * r_v),
        "J_phi": W_PHI * DOMAIN_AREA * torch.mean(r_phi * r_phi),
        "J_T": W_T * DOMAIN_AREA * torch.mean(r_T * r_T),
    }
...
def write_readme(path, args):
    lines = []
    lines.append("case01_drm baseline")
    lines.append("This script implements a Deep Ritz Method baseline for the same manufactured-solution PDE case used in the previous case.")
    ...
    lines.append("This implementation does not force a classical scalar Deep Ritz energy for the coupled convective-pressure system.")
    ...
    lines.append("Domain integral uses plain uniform interior sampling and area-weighted Monte Carlo approximation.")
    lines.append("Boundary integral uses plain uniform boundary sampling and total-boundary-length-weighted Monte Carlo approximation.")
    lines.append("This script stays on the variational or energy-minimization route and does not introduce pointwise adaptive PICS mechanisms.")
...
def main():
    parser = argparse.ArgumentParser(description="DRM least-squares variational baseline for case01 manufactured PDE system.")
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    parser.add_argument("--device", type=str, default=DEVICE_DEFAULT)
    parser.add_argument("--outdir", type=str, default="case01_drm")
    ...
    readme_path = os.path.join(logs_dir, "README_case01_drm.txt")
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        domain_terms = residual_squares(model, xy_f, rho_e, f_u, f_v, f_T)
    ...
    for field_name in ["u", "v", "p", "phi", "T"]:
        ...
        metric_lines.append(
            f"{field_name} {mse:.16e} {rmse:.16e} {mae:.16e} {rel_l2:.16e} {max_err:.16e}"
        )

    metrics_path = os.path.join(logs_dir, "metrics_case01_drm.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("\n".join(metric_lines))

    runtime_path = os.path.join(logs_dir, "runtime_case01_drm.txt")
