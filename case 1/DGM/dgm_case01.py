...
class DGMNet(nn.Module):
    ...
    def forward(self, xy):
        # DGM starts from an input feature state and repeatedly applies
        # gated recurrent updates driven by the original coordinates.
...
def boundary_points(device, dtype):
    ...
    truth = manufactured_fields(xy_b)
    return xy_b, truth
...
def pde_residuals(model, xy, rho_e, f_u, f_v, f_T):
    xy_req = xy.clone().detach().requires_grad_(True)
    raw = model(xy_req)
    fields = scale_outputs(raw)
...
def write_readme(path, args):
    lines = []
    lines.append("case01_dgm baseline")
    lines.append("This script implements a Deep Galerkin Method baseline for the same manufactured-solution PDE case used in the previous case.")
    ...
    lines.append("DGM principle: a gated recurrent deep network approximates the PDE solution fields, and training directly minimizes strong-form PDE residuals together with boundary-condition mismatch.")
    lines.append("This script keeps only the original DGM solving idea and does not include MoE, gate routing, certificate logic, replay buffers, adaptive point insertion, or any custom enhancement module from the newer method.")
    lines.append("core hyperparameters:")
    lines.append(f"seed = {args.seed}")
    lines.append(f"device = {args.device}")
    lines.append(f"dtype = torch.float64")
...
def main():
    parser = argparse.ArgumentParser(description="DGM baseline for case01 manufactured PDE system.")
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    parser.add_argument("--device", type=str, default=DEVICE_DEFAULT)
    parser.add_argument("--outdir", type=str, default="case01_dgm")
    ...
    readme_path = os.path.join(logs_dir, "README_case01_dgm.txt")
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        phys_losses = pde_residuals(model, xy_f, rho_e, f_u, f_v, f_T)
    ...
    metrics_path = os.path.join(logs_dir, "metrics_case01_dgm.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("\n".join(metric_lines))

    runtime_path = os.path.join(logs_dir, "runtime_case01_dgm.txt")
