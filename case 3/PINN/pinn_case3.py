...
def write_readme(path, args):
    lines = []
    lines.append("case10_pinn baseline")
    lines.append("This script implements the most primitive vanilla PINN baseline for the same Case10 manufactured-solution PDE used in the previous Case10 workflow.")
    ...
    lines.append("Case10 residual structure includes screened-Poisson electrostatics and pressure-gradient regularization through third-order derivatives.")
...
def main():
    parser = argparse.ArgumentParser(description="Vanilla PINN baseline for Case10 manufactured PDE system.")
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    parser.add_argument("--device", type=str, default=DEVICE_DEFAULT)
    parser.add_argument("--outdir", type=str, default="case10_pinn")
    ...
    write_readme(os.path.join(logs_dir, "README_case10_pinn.txt"), args)
    ...
    with open(os.path.join(logs_dir, "metrics_case10_pinn.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(metric_lines))

    with open(os.path.join(logs_dir, "runtime_case10_pinn.txt"), "w", encoding="utf-8") as f:
