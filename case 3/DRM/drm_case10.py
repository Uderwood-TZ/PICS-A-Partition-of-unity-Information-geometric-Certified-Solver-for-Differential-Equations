...
class MLPNet(nn.Module):
    ...
    def __init__(self, in_dim: int = 2, out_dim: int = 5, width: int = WIDTH, depth: int = DEPTH):
        super().__init__()
...
def sx_variable(x: torch.Tensor, y: torch.Tensor):
...
def sy_variable(x: torch.Tensor, y: torch.Tensor):
...
def combo_variable(x: torch.Tensor, y: torch.Tensor):
...
def manufactured_fields(xy: torch.Tensor):
    x = xy[:, 0:1]
    y = xy[:, 1:2]
...
def grad_scalar(field: torch.Tensor, xy: torch.Tensor) -> torch.Tensor:
...
def second_partials(field: torch.Tensor, xy: torch.Tensor):
...
def pressure_derivative_terms(field: torch.Tensor, xy: torch.Tensor):
    grad_field = grad_scalar(field, xy)
    p_x = grad_field[:, 0:1]
    p_y = grad_field[:, 1:2]
...
def manufactured_sources(xy: torch.Tensor):
...
def scale_outputs(raw: torch.Tensor):
    u_tilde = raw[:, 0:1]
    v_tilde = raw[:, 1:2]
    p_tilde = raw[:, 2:3]
    phi_tilde = raw[:, 3:4]
...
def uniform_interior_pool(n_points: int, device: torch.device, dtype: torch.dtype):
    xy = np.random.rand(n_points, 2)
    return torch.tensor(xy, dtype=dtype, device=device)


def boundary_points(device: torch.device, dtype: torch.dtype):
...
def batch_indices(n_total: int, batch_size: int):
...
def build_interior_pool(device: torch.device, dtype: torch.dtype):
...
def drm_functional_terms(model: nn.Module, xy: torch.Tensor, rho_e: torch.Tensor, f_u: torch.Tensor, f_v: torch.Tensor, f_T: torch.Tensor):
    xy_req = xy.clone().detach().requires_grad_(True)
...
def boundary_functional(model: nn.Module, xy_b: torch.Tensor, truth_b):
...
def evaluate_model(model: nn.Module, xy: torch.Tensor, device: torch.device, dtype: torch.dtype):
    model.eval()
    n_total = xy.shape[0]
    pred = {"u": [], "v": [], "p": [], "phi": [], "T": []}
    true = {"u": [], "v": [], "p": [], "phi": [], "T": []}
...
def save_field_txt(path: str, X: np.ndarray, Y: np.ndarray, V: np.ndarray) -> None:
    data = np.column_stack([X.reshape(-1), Y.reshape(-1), V.reshape(-1)])
    np.savetxt(path, data, fmt="%.16e")


def plot_field(path: str, X: np.ndarray, Y: np.ndarray, V: np.ndarray, title: str, vmin: float, vmax: float) -> None:
...
def compute_metrics(true_vec: np.ndarray, pred_vec: np.ndarray):
...
def write_readme(path: str, args) -> None:
    lines = []
    lines.append("case10_drm baseline")
    lines.append("This script implements a Deep Ritz Method baseline for the same Case10 manufactured-solution PDE used in the previous Case10 workflow.")
    ...
    lines.append("This implementation does not force a classical scalar Deep Ritz energy for the coupled screened-Poisson and pressure-gradient-regularized PDE system.")
    ...
    lines.append("Case10 enters the variational functional through screened-Poisson electrostatics and pressure-gradient regularization:")
    lines.append("rho_e = -EPS*(phi_xx + phi_yy) + KAPPA_D^2*phi")
    lines.append("r_u contains C_P*(p_xxx + p_xyy), and r_v contains C_P*(p_yxx + p_yyy)")
    ...
    lines.append("The script uses fixed affine output scaling to align the output convention with the Case10 baseline family.")
...
def plot_losses(loss_history_path: str, figs_dir: str) -> None:
...
def print_file_tree(root_dir: str) -> None:
    print("\nGenerated files:")
...
def main():
    parser = argparse.ArgumentParser(description="Least-squares variational DRM baseline for Case10 manufactured PDE system.")
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    parser.add_argument("--device", type=str, default=DEVICE_DEFAULT)
    parser.add_argument("--outdir", type=str, default="case10_drm")
    ...
    write_readme(os.path.join(logs_dir, "README_case10_drm.txt"), args)
    ...
    with open(os.path.join(logs_dir, "metrics_case10_drm.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(metric_lines))

    with open(os.path.join(logs_dir, "runtime_case10_drm.txt"), "w", encoding="utf-8") as f:
