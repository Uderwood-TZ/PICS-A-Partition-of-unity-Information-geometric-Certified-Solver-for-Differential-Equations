...
# Case09 manufactured thin layer + kernels
DELTA = 0.018   # S-layer thickness

# Leray smoothing scale (Taylor approx)
L_SMOOTH = 0.02
...
def x0_of_y(y: torch.Tensor) -> torch.Tensor:
    return 0.52 + 0.02 * torch.sin(2.0 * PI * y)
...
# ============================================================
# Case09 manufactured truth (ONLY for RHS, BC, evaluation)
# ============================================================

def x_s_of_y(y: torch.Tensor) -> torch.Tensor:
    return 0.5 + 0.12 * (y - 0.5) + 0.08 * torch.sin(2.0 * PI * y)


def s_layer(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    xs = x_s_of_y(y)
    return torch.tanh((x - xs) / DELTA)


def g_hot(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    return torch.exp(-((x - 0.25) ** 2 + (y - 0.75) ** 2) / 0.02)


def g_cold(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
...
def psi_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    return (
        ...
        + 0.02 * torch.cos(3.0 * PI * x) * torch.sin(PI * y)
    )


def p_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    sS = s_layer(x, y)
    return 1.0 + 0.24 * sS + 0.05 * torch.cos(2.0 * PI * y) + 0.012 * torch.sin(2.0 * PI * x + 0.6 * PI * y)


def phi_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    sS = s_layer(x, y)
    return (
        0.36 * torch.sin(2.0 * PI * x) * torch.sin(2.0 * PI * y)
        + 0.16 * sS * torch.cos(PI * y)
        + 0.02 * torch.sin(3.0 * PI * x) * torch.sin(PI * y)
        + 0.012 * torch.cos(2.0 * PI * x + 0.6 * PI * y)
    )
...
def T_star(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    return (
        300.0
        + 12.0 * sS
        + 6.0 * torch.sin(2.0 * PI * x) * torch.cos(2.0 * PI * y)
        + 3.0 * g_hot(x, y)
        - 2.5 * g_cold(x, y)
        + 0.6 * torch.sin(2.0 * PI * x + 0.6 * PI * y)
    )
...
def truth_rho_and_sources(x_in: torch.Tensor, y_in: torch.Tensor, create_graph: bool):
    ...
    flux_ux = nu_T * u_x
    flux_uy = nu_T * u_y
    diff_u = grad(flux_ux, x, create_graph=True) + grad(flux_uy, y, create_graph=True)

    flux_vx = nu_T * v_x
    flux_vy = nu_T * v_y
    diff_v = grad(flux_vx, x, create_graph=True) + grad(flux_vy, y, create_graph=True)
...
def residuals_normalized(pred: dict,
    ...
    nu_T = NU0 * torch.exp(-A_T * pred["T_tilde"])

    flux_ux = nu_T * pred["u_x"]
    flux_uy = nu_T * pred["u_y"]
    diff_u = grad(flux_ux, pred["_x"], create_graph=True) + grad(flux_uy, pred["_y"], create_graph=True)

    flux_vx = nu_T * pred["v_x"]
    flux_vy = nu_T * pred["v_y"]
    diff_v = grad(flux_vx, pred["_x"], create_graph=True) + grad(flux_vy, pred["_y"], create_graph=True)

    r_u = ubar * pred["u_x"] + vbar * pred["u_y"] + pred["p_x"] - diff_u + ALPHA_E * rho_e * pred["phi_x"] - f_u
    r_v = ubar * pred["v_x"] + vbar * pred["v_y"] + pred["p_y"] - diff_v + ALPHA_E * rho_e * pred["phi_y"] - f_v
...
def sample_band_slayer(rng: np.random.Generator, n: int) -> np.ndarray:
    y = rng.random((n, 1))
    xs = 0.5 + 0.12 * (y - 0.5) + 0.08 * np.sin(2.0 * PI * y)
    dx = (2.0 * rng.random((n, 1)) - 1.0) * BAND_W
    x = np.clip(xs + dx, 0.0, 1.0)
...
def build_initial_interior_pool(rng: np.random.Generator, nf_init: int) -> np.ndarray:
    ...
    xy_band = sample_band_slayer(rng, n_band)
...
def main():
    ...
    # Precompute RHS on certificate grid (autograd, fixed RHS)
    t0_rhs = now_s()
    ...
    def certificate_scan_and_adapt(step: int):
        ...
        add_band = sample_band_slayer(rng, ADD_BAND_PER_SCAN)
    ...
    metrics_path = os.path.join(logs_dir, "metrics_case09.txt")
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("PICS Case09 v3.1 metrics\n")
    ...
    runtime_path = os.path.join(logs_dir, "runtime_case09.txt")
    ...
    readme_path = os.path.join(logs_dir, "README_case09.txt")
    with open(readme_path, "w", encoding="utf-8") as f:
        f.write("PICS Case09 v3.1 README\n\n")
        f.write("Constraints:\n")
        ...
        f.write("Case09 PDE (Leray + thermo-viscous):\n")
        f.write("- Leray convecting velocity smoothing (Taylor): ubar=u+0.5*l^2*Δu, vbar=v+0.5*l^2*Δv.\n")
        f.write("- Thermo-viscous variable viscosity: nu_T = nu0*exp(-a_T*T_tilde), diffusion div(nu_T∇u), div(nu_T∇v).\n\n")
        f.write("PICS v3.1 mechanisms (same as Case01–08):\n")
        ...
        f.write("Sampling:\n")
        f.write("- Band sampling targets |x-x_s(y)|<=band_w with direct construction; always returns (n,2).\n\n")
        f.write("Outputs:\n")
        f.write("- case09/figs: 15 jet plots + loss/cert curves.\n")
        f.write("- case09/data: 15 txt triplets x y value.\n")
        f.write("- case09/logs: metrics_case09.txt runtime_case09.txt loss_history.txt README_case09.txt.\n")
