...
def write_readme(path, args):
    lines = []
    lines.append("case09_dgm baseline")
    lines.append("This script implements an original-style Deep Galerkin Method baseline for the same Case09 manufactured-solution PDE used in the previous Case09 workflow.")
    ...
    lines.append("The current implementation keeps the Case09 Leray-smoothed convective velocity and temperature-dependent viscosity.")
...
def main():
    parser = argparse.ArgumentParser(description="Original-style DGM baseline for Case09 manufactured PDE system.")
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    parser.add_argument("--device", type=str, default=DEVICE_DEFAULT)
    parser.add_argument("--outdir", type=str, default="case09_dgm")
    ...
    write_readme(os.path.join(logs_dir, "README_case09_dgm.txt"), args)
    ...
    with open(os.path.join(logs_dir, "metrics_case09_dgm.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(metric_lines))

    with open(os.path.join(logs_dir, "runtime_case09_dgm.txt"), "w", encoding="utf-8") as f:
