...
def write_readme(path, args):
    lines = []
    lines.append("case09_pinn baseline")
    lines.append("This script implements the most primitive vanilla PINN baseline for the same Case09 manufactured-solution PDE used in the previous Case09 workflow.")
    ...
    lines.append("The continuity equation enters the loss as an ordinary residual term. The script does not use any streamfunction hard constraint.")
    lines.append("Case09 residual structure includes Leray-smoothed convective velocity and temperature-dependent viscosity.")
    lines.append("Specifically, ubar = u + 0.5*L_SMOOTH^2*(u_xx + u_yy), vbar = v + 0.5*L_SMOOTH^2*(v_xx + v_yy), and nu_T = NU0*exp(-A_T*(T-T0)/T1).")
    lines.append("The viscous diffusion enters as diff_u = d/dx(nu_T*u_x) + d/dy(nu_T*u_y) and diff_v = d/dx(nu_T*v_x) + d/dy(nu_T*v_y).")
    lines.append("No adaptive sampling, replay buffer, certificate, gate, MoE, Fourier feature, hard constraint transform, residual connection, or interior supervised data is used.")
...
def main():
    parser = argparse.ArgumentParser(description="Vanilla PINN baseline for Case09 manufactured PDE system.")
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    parser.add_argument("--device", type=str, default=DEVICE_DEFAULT)
    parser.add_argument("--outdir", type=str, default="case09_pinn")
    ...
    write_readme(os.path.join(logs_dir, "README_case09_pinn.txt"), args)
    ...
    with open(os.path.join(logs_dir, "metrics_case09_pinn.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(metric_lines))

    with open(os.path.join(logs_dir, "runtime_case09_pinn.txt"), "w", encoding="utf-8") as f:
